...
function F_CalendarFooter({
  ...
  datePickerTimePickerLabel, // Time picker
}: FooterProps): JSX.Element {
  const { t } = useTranslation();

  const timepickerId = useId();
  const format24hoursId = useId();
  const hoursId = useId();
      ...
      <div className={footerWrapperClassName}>
        ...
        {isTimePickerEnabled ? (
          <div className={checkboxMarkerContainerClassName}>
            ...
            <label htmlFor={format24hoursId} className={calendarLabelClassName}>
              {t(hoursFormatLabel)}
            </label>
          </div>
        ) : null}
