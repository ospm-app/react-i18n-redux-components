...
import { ComboboxOption } from 'library/intl/timezone/combobox/combobox-option.tsx';
...
>({
  ...
  useEffect((): void => {
    let index = options.findIndex((option): boolean => {
      return option.value === field.selectedTimezone;
    });
  ...
  }, [open, options, field.selectedTimezone]);
  ...
  const toggleDropdown = useCallback<MouseEventHandler<HTMLInputElement>>(
    (event) => {
      ...
      if (
        ...
        typeof field.selectedTimezone !== 'undefined'
      ) {
        onFocus(field.selectedTimezone, field);
      }
  ...
  const onLiKeyDown = useCallback<KeyboardEventHandler<HTMLLIElement>>(
    (event): void => {
      event.stopPropagation();
  ...
  const onDivBlur = useCallback<FocusEventHandler<HTMLDivElement>>(
    (event) => {
      ...
      if (typeof onSelectBlur === 'function') {
        ...
        const value =
          typeof field.selectedTimezone !== 'undefined'
            ? field.selectedTimezone
            : typeof option !== 'undefined'
              ? option.value
              : undefined;

        if (typeof value !== 'undefined') {
          onSelectBlur(value, field);
        }
      }
    },
  ...
  const currentOption = useMemo<
    Readonly<SelectOptionIntl<Value>> | undefined
  >(() => {
    const value = field.selectedTimezone;

    return options.find((option): boolean => {
      return option.value === value;
    });
  }, [field.selectedTimezone, options]);
  ...
  const readonlyInputValue = useMemo<string>(() => {
    ...
    return `${t(currentOption.label)} ${currentOption['offset']} (${currentOption['time']})`;
  ...
  return (
    <div ref={ref} className={selectDivClassName} onBlur={onDivBlur}>
      ...
      >
        ...
        {options.map(
          (option: SelectOptionIntl<Value>, index: number): JSX.Element => {
            return (
...
export const TimezoneCombobox = reactMemo(F_TimezoneCombobox);
