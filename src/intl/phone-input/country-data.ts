...
import type { Country, RawCountry } from 'library/intl/phone-input/types.ts';

const rawAllCountries: ReadonlyArray<RawCountry> = [
...
const allCountryCodes: Record<string, Array<string>> = {};
...
function addCountryCode(
  ...
  const codes = allCountryCodes[dialCode];
...
function generateCountry(country: Readonly<RawCountry>): Array<Country> {
  const countryItem: Country = {
    ...
    hasAreaCodes: Boolean(country[6]),
  };

  const areaItems: Array<Country> = [];

  if (country[6]) {
    for (const areaCode of country[6]) {
  ...
  addCountryCode(countryItem.iso2, countryItem.dialCode, countryItem.priority);

  return areaItems.length > 0 ? [countryItem, ...areaItems] : [countryItem];
}

export const allCountries = rawAllCountries.reduce<Array<Country>>(
  (acc, item): Array<Country> => {
    acc.push(...generateCountry(item));
